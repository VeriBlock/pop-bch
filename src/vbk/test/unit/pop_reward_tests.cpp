...
BOOST_FIXTURE_TEST_CASE(addPopPayoutsIntoCoinbaseTx_test, PopRewardsTestFixture)
{
    ...
    spending.vout[0].nValue = ((int64_t)100) * Amount::satoshi();
    spending.vout[0].scriptPubKey = scriptPubKey;

    std::vector<unsigned char> vchSig;
    uint256 hash = SignatureHash(scriptPubKey, spending, 0, SigHashType().withForkId(), payoutBlock.vtx[0]->vout[1].nValue);
