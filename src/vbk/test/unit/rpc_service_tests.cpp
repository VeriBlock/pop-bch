...
BOOST_FIXTURE_TEST_CASE(extractblockinfo_test, E2eFixture) {
    ...
    BOOST_CHECK_EQUAL(result[0]["hash"].get_str(),
                      tip->GetBlockHash().GetHex());
    BOOST_CHECK_EQUAL(result[0]["height"].get_int64(), tip->nHeight);
    BOOST_CHECK_EQUAL(result[0]["previousHash"].get_str(),
                      tip->GetBlockHeader().hashPrevBlock.GetHex());
    BOOST_CHECK_EQUAL(
        result[0]["previousKeystone"].get_str(),
        uint256(container.keystones.firstPreviousKeystone).GetHex());
    BOOST_CHECK_EQUAL(
        result[0]["secondPreviousKeystone"].get_str(),
