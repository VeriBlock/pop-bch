...
bool AltChainParamsVBCH::checkBlockHeader(const std::vector<uint8_t>& bytes, const std::vector<uint8_t>& root, altintegration::ValidationState& state) const noexcept
{
...
std::vector<uint8_t> AltChainParamsVBCH::getHash(const std::vector<uint8_t>& bytes) const noexcept
{
    try {
