...
UniValue extractblockinfo(const Config &, const JSONRPCRequest &req) {
    ...
    auto encoded_bytes =
        ParseHexV(req.params[0].get_str(), "publication data sbytes");
    ...
    altintegration::PublicationData pubData;
    {
        ...
        if (!altintegration::DeserializeFromVbkEncoding(stream, pubData,
                                                        state)) {
            return JSONRPCError(RPC_INVALID_PARAMETER,
    ...
    res.pushKV("previousKeystone",
               HexStr(container.keystones.firstPreviousKeystone));
    res.pushKV("secondPreviousKeystone",
