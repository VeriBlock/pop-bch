...
template <typename BlockT> std::pair<char, std::string> tip_key();

template <> inline std::pair<char, std::string> tip_key<VbkBlock>() {
    return std::make_pair(DB_VBK_TIP, "vbktip");
}
template <> inline std::pair<char, std::string> tip_key<BtcBlock>() {
    return std::make_pair(DB_BTC_TIP, "btctip");
}
template <> inline std::pair<char, std::string> tip_key<AltBlock>() {
    return std::make_pair(DB_ALT_TIP, "alttip");
}

template <typename BlockT>
std::pair<char, typename BlockT::hash_t>
block_key(const typename BlockT::hash_t &hash);

template <>
inline std::pair<char, typename BtcBlock::hash_t>
block_key<BtcBlock>(const typename BtcBlock::hash_t &hash) {
    return std::make_pair(DB_BTC_BLOCK, hash);
}

template <>
inline std::pair<char, typename VbkBlock::hash_t>
block_key<VbkBlock>(const typename VbkBlock::hash_t &hash) {
    return std::make_pair(DB_VBK_BLOCK, hash);
}

template <>
inline std::pair<char, typename AltBlock::hash_t>
block_key<AltBlock>(const typename AltBlock::hash_t &hash) {
    return std::make_pair(DB_ALT_BLOCK, hash);
}

template <typename BlockT>
std::pair<char, typename BlockT::prev_hash_t>
block_prev_key(const typename BlockT::prev_hash_t &hash);

template <>
inline std::pair<char, typename BtcBlock::prev_hash_t>
block_prev_key<BtcBlock>(const typename BtcBlock::prev_hash_t &hash) {
    return std::make_pair(DB_BTC_PREV_BLOCK, hash);
}

template <>
inline std::pair<char, typename VbkBlock::prev_hash_t>
block_prev_key<VbkBlock>(const typename VbkBlock::prev_hash_t &hash) {
    return std::make_pair(DB_VBK_PREV_BLOCK, hash);
}

template <>
inline std::pair<char, typename AltBlock::prev_hash_t>
block_prev_key<AltBlock>(const typename AltBlock::prev_hash_t &hash) {
...
struct BlockReader : public altintegration::BlockReader {
    ...
    bool getBlock(const AltBlock::prev_hash_t &prev_hash,
        ...
        return _db->Read(block_prev_key<AltBlock>(prev_hash), hash) &&
               _db->Read(block_key<AltBlock>(hash), out);
    }

    bool getBlock(const VbkBlock::prev_hash_t &prev_hash,
                  StoredBlockIndex<VbkBlock> &out) const override {
        VbkBlock::hash_t hash;
        return _db->Read(block_prev_key<VbkBlock>(prev_hash), hash) &&
               _db->Read(block_key<VbkBlock>(hash), out);
    }

    bool getBlock(const BtcBlock::prev_hash_t &prev_hash,
                  StoredBlockIndex<BtcBlock> &out) const override {
        BtcBlock::hash_t hash;
        return _db->Read(block_prev_key<BtcBlock>(prev_hash), hash) &&
               _db->Read(block_key<BtcBlock>(hash), out);
    }

    std::shared_ptr<altintegration::BlockIterator<AltBlock>>
    ...
    std::shared_ptr<altintegration::BlockIterator<VbkBlock>>
    getVbkBlockIterator() const override {
        std::shared_ptr<CDBIterator> it(_db->NewIterator());
        return std::make_shared<BlockIterator<VbkBlock>>(it);
    }
    std::shared_ptr<altintegration::BlockIterator<BtcBlock>>
...
struct BlockBatch : public altintegration::BlockBatch {
    ...
    void writeBlock(const AltBlock::hash_t &hash,
                    ...
                    const StoredBlockIndex<AltBlock> &value) override {
    ...
    void writeBlock(const VbkBlock::hash_t &hash,
                    ...
                    const StoredBlockIndex<VbkBlock> &value) override {
    ...
    void writeBlock(const BtcBlock::hash_t &hash,
                    ...
                    const StoredBlockIndex<BtcBlock> &value) override {
