...
CBlock CreateGenesisBlock(std::string pszTimestamp,
                          const CScript &genesisOutputScript, uint32_t nTime,
                          uint32_t nNonce, uint32_t nBits, int32_t nVersion,
    ...
    txNew.vin[0].scriptSig =
        VeriBlock::ScriptWithPrefix(nBits)
        << std::vector<uint8_t>{pszTimestamp.begin(), pszTimestamp.end()};
...
CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits,
    ...
    const CScript genesisOutputScript = CScript() << ParseHex(initialPubkeyHex)
                                                  << OP_CHECKSIG;
    return VeriBlock::CreateGenesisBlock(pszTimestamp, genesisOutputScript,
